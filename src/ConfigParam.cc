...
bool ConfigParam::_saveVIEstimates = true;
double ConfigParam::_MinVIOInitTime = 15.0;
...
ConfigParam::ConfigParam(std::string configfile) {
  ...
  fSettings["test.saveVIEstimates"] >> _saveVIEstimates;
  fSettings["test.InitVIOTmpPath"] >> _tmpFilePath;
  if (_saveVIEstimates) {
    std::cout << "save tmp file in "<<_tmpFilePath<<std::endl;
  } else {
    std::cout << "test.saveVIEstimates = false, not saving tmp file" << std::endl;
  }

  // Comment out ROS variables
  ...
  _MinVIOInitTime = fSettings["LocalMapping.MinVIOInitTime"];
  std::cout << "Minimum VIO Init Time: " << _MinVIOInitTime << std::endl;
...
bool ConfigParam::getSaveVIEstimates() {
  return _saveVIEstimates;
}
...
double ConfigParam::GetMinVIOInitTime() {
  return _MinVIOInitTime;
}
