...
namespace ORB_SLAM2
{
  ...
  Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, 
                     ...
                     const string &strSettingPath, const int sensor, const bool bUseImu):
    mbCreateNewKFAfterReloc(false), mbRelocBiasPrepare(false),
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), bUseImu(bUseImu),
  ...
  Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), bUseImu(false),
    mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), 
    mpSystem(pSys), mpViewer(NULL),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
  ...
  // Predict the pose of the current frame by the amount of 
  // IMU pre-integration from the previous key frame to this frame
  void Tracking::PredictNavStateByIMU(bool bMapUpdated) {
    if (!mpLocalMapper->GetVINSInited()) {
      cerr << "mpLocalMapper->GetVINSInited() is false, shouldn't in PredictNavStateByIMU" << endl;
    }
    // Map updated, optimize with last KeyFrame
    if (mpLocalMapper->GetFirstVINSInited() || bMapUpdated) {
      if (mpLocalMapper->GetFirstVINSInited() && !bMapUpdated) {
        cerr << "2-FirstVinsInit, but not bMapUpdated. shouldn't" << endl;
      }

      // Compute IMU Pre-integration
      mIMUPreIntInTrack = GetIMUPreIntSinceLastKF(&mCurrentFrame, 
                                                  mpLastKeyFrame, 
                                                  mvIMUSinceLastKF);

      // Get initial NavState&pose from Last KeyFrame
      mCurrentFrame.SetInitialNavStateAndBias(mpLastKeyFrame->GetNavState());
      mCurrentFrame.UpdateNavState(mIMUPreIntInTrack, 
                                   Converter::toVector3d(mpLocalMapper->GetGravityVec()));
  ...
  // Repace the original TrackWithMotionModel();
  bool Tracking::TrackWithIMU(bool bMapUpdated) {
    ...
    if (!mpLocalMapper->GetVINSInited()) {
      cerr << "local mapping VINS not inited but you are in TrackWithIMU!" << endl;
    }

    // Predict NavState&Pose by IMU
    // And compute the IMU pre-integration for PoseOptimization
    // (replace) mCurrentFrame.SetPose(mVelocity * mLastFrame.mTcw);
  ...
  cv::Mat Tracking::GrabImageMonoImu(const cv::Mat &im, const double &timestamp,
                                     const std::vector<ORB_SLAM2::IMUData> &vBufferImuData) {
    // Append IMU Data
    mvIMUSinceLastKF.insert(mvIMUSinceLastKF.end(), vBufferImuData.begin(), vBufferImuData.end());
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) {
      mCurrentFrame = Frame(mImGray, timestamp, vBufferImuData, mpIniORBextractor, 
                            mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
    } else {
      mCurrentFrame = Frame(mImGray, timestamp, vBufferImuData, mpORBextractorLeft,
                            mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
    }
  ...
  void Tracking::Track() {
    ...
    if (mState==NOT_INITIALIZED) {
        ...
        if (mState == OK) {
          ...
          // if (mpLocalMapper->GetScale() != 1.0) {
            ...
            if (mbRelocBiasPrepare) {
              ...
              bOK = TrackWithIMU(bMapUpdated);  //  replaced TrackWithMotionModel();
              if (!bOK)
                bOK = TrackReferenceKeyFrame();
            }
          } else {
            // If Visual-Intertial not initialized, keep same as pure vslam
      ...
      if (!mbOnlyTracking) {
        if (bOK) {
          if (!bUseImu || !mpLocalMapper->GetVINSInited()) {  // || mbRelocBiasPrepare) {
            bOK = TrackLocalMap();
          } else {
      ...
      if (bOK) {
        mState = OK;
        // Add Frames to re-compute IMU bias afer reloc
        if (mbRelocBiasPrepare) {
  ...
  bool Tracking::TrackLocalMapWithIMU (bool bMapUpdated) {
    cout << "Inside tracklocalmapwithimu" << endl;
  ...
  IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF,
                                                     KeyFrame* pLastKF, 
  ...
  void Tracking::CreateNewKeyFrame()
  {
    ...
    KeyFrame* pKF = new KeyFrame(mCurrentFrame, mpMap, mpKeyFrameDB, 
                                 mvIMUSinceLastKF, mpLastKeyFrame);
