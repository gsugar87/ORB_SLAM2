...
namespace ORB_SLAM2
{
  ...
  void LocalMapping::DeleteBadInLocalWindow() {
  ...
  bool LocalMapping::TryInitVIO() {
    // Make sure there are enough keyframes in the local window
    ...
    if(!fopened && ConfigParam::getSaveVIEstimates()) {
      ...
      if(fgw.is_open() && fscale.is_open() && fbiasa.is_open() &&
        ...
        cerr << "file open error in TryInitVIO.  Couldn't open " << 
          tmpfilepath << endl;
        fopened = false;
      }
    ...
    for (int i=0; i<N-2; i++) {
      ...
      if (dt12!=pKF2->mTimeStamp-pKF1->mTimeStamp) {
        ...
        cerr<<"dt12= "<< dt12 << "pKF2->mTimeStamp= " << pKF2->mTimeStamp <<
          " pKF1->mTimeStamp= " << pKF1->mTimeStamp << 
          "pKF2->mTimeStamp-pKF1->mTimeStamp= " <<
          pKF2->mTimeStamp-pKF1->mTimeStamp << endl;
      }
      if (dt23!=pKF3->mTimeStamp-pKF2->mTimeStamp) {
        cerr<<"dt23!=pKF3->mTimeStamp-pKF2->mTimeStamp"<<endl;
        cerr<<"dt23= "<< dt23 << "pKF3->mTimeStamp= " << pKF3->mTimeStamp <<
          " pKF2->mTimeStamp= " << pKF2->mTimeStamp << 
          "pKF3->mTimeStamp-pKF2->mTimeStamp= "<<
          pKF3->mTimeStamp-pKF2->mTimeStamp << endl;
      }
    ...
    if(w.type()!=I3.type() || u.type()!=I3.type() || vt.type()!=I3.type()) {
      cerr << "different mat type, I3,w,u,vt: " << I3.type() << "," << 
        w.type()<<","<<u.type()<<","<<vt.type()<<endl;
    }
    ...
    // Debug log
    {
      ...
      if (ConfigParam::getSaveVIEstimates()) {
  ...
  bool LocalMapping::TryInitStereoVIO() {
    cerr << "Stereo + IMU not working!" << endl;
    return false;
  }
          ...
          if (mpMap->KeyFramesInMap()>2) {
            if (!GetVINSInited()) {
              ...
              Optimizer::LocalBundleAdjustmentNavState(mpCurrentKeyFrame, mlLocalKeyFrames,
                                                       &mbAbortBA, mpMap, mGravityVec, this);
            }
          }
          
          // Try to initialize VIO if not inited
          if (mbUseImu && !GetVINSInited()) {
            bool initSuccess;
            if (!mbMonocular) {
              initSuccess = TryInitStereoVIO();
            } else {
  ...
  void LocalMapping::KeyFrameCulling()
  {
    ...
    for (vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), 
      ...
      // if (pKF->mnId >= pOldestLocalKF->mnId || pKF->mnId >= pPrevLocalKF->mnId)
      //  continue;
